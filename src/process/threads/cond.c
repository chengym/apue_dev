...
int main(void)
{
    ...
    pthread_join(t_c, NULL);/*等待进程t_b结束*/
...
void *thread1(void *junk)
{
    for(i=1;i<=6;i++)
    {
        ...
        if(i%3==0){
            ...
            pthread_cond_broadcast(&cond);/*条件改变，发送信号，通知t_b进程*/
...
void *thread3(void *junk)
{
    ...
    {
        pthread_mutex_lock(&mutex);
        printf("thread3: lock %d\n", __LINE__);
        if(i%3!=0){
            printf("thread3: wait 1  %d\n", __LINE__);
            pthread_cond_wait(&cond,&mutex);/*解锁mutex，并等待cond改变*/
            printf("thread3: wait 2  %d\n", __LINE__);
        }
        pthread_mutex_unlock(&mutex);
        printf("thread3: unlock %d\n\n", __LINE__);
        sleep(1);
    }
}
